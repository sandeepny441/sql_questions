...
def run_forecast_pipeline(
    ...
    forecast_df, data_quality_notes = _prepare_dataframe(df, run_month, bucket_filter)
...
def _prepare_dataframe(
    ...
    prepared = df.copy()
    data_quality_notes = []
    prepared["_source_row_id"] = prepared.index.astype(str)
    ...
    if normalized_bucket_filter is not None:
        prepared = prepared[prepared["bucket"].isin(normalized_bucket_filter)].copy()
        if prepared.empty:
    ...
    if prepared["_month_dt"].isna().any():
        dropped_count = int(prepared["_month_dt"].isna().sum())
        prepared = prepared.loc[prepared["_month_dt"].notna()].copy()
        data_quality_notes.append(
            f"Dropped {dropped_count} rows because month could not be parsed."
        )
        if prepared.empty:
            raise ValueError("No usable rows remain after dropping invalid month values.")
    ...
    duplicate_mask = prepared.duplicated(subset=["nmls", "_month_dt"], keep="last")
    if duplicate_mask.any():
        duplicate_count = int(duplicate_mask.sum())
        prepared = prepared.loc[~duplicate_mask].copy()
        data_quality_notes.append(
            f"Collapsed {duplicate_count} duplicate nmls-month rows by keeping the last occurrence."
        )

    prepared = prepared.sort_values(["nmls", "_month_dt"]).reset_index(drop=True)
    return prepared, data_quality_notes


def _normalize_nmls(value) -> str:
    if pd.isna(value):
        return ""
...
def _normalize_bucket_filter(bucket_filter):
    ...
    return {_normalize_bucket(value) for value in bucket_values}
...
def _append_missing_future_rows(df: pd.DataFrame, run_month: str | None = None) -> pd.DataFrame:
    available_months = sorted(df["_month_dt"].dropna().unique().tolist(), key=lambda x: str(x))
...
def _mean(values) -> float:
    clean_values = [_safe_number(value, default=np.nan) for value in values]
    clean_values = [value for value in clean_values if np.isfinite(value)]
    return float(sum(clean_values) / len(clean_values)) if clean_values else 0.0
...
def _signed_delta_pct(forecast_value, actual_value):
    forecast_value = _safe_number(forecast_value, default=np.nan)
    actual_value = _safe_number(actual_value, default=np.nan)
    if not np.isfinite(forecast_value) or not np.isfinite(actual_value) or actual_value == 0:
        return 0.0
...
def _build_training_months(rows_by_nmls, cutoff_month):
    ...
    return sorted(months, key=lambda x: str(x))
...
def _recursive_market_forecast(
    ...
    ordered_months = sorted(months_to_predict, key=lambda x: str(x))
    forecast_results = {}
...
def _apply_ml_market_forecast(records, rows_by_nmls, rows_by_month, run_month=None):
    ...
    for nmls_rows in rows_by_nmls.values():
        for month in sorted(windows["test_months"] + windows["future_months"], key=lambda x: str(x)):
            if month not in nmls_rows:
                continue
...
def _build_bucket_error_summaries(records, test_month_labels):
    ...
    for metric_name, rule_delta_key, ml_delta_key in metric_configs:
        ...
        remaining_buckets = sorted(
            (set(bucket_rule_deltas) | set(bucket_ml_deltas)) - set(BUCKET_ORDER),
            key=lambda x: str(x),
        )
        summary_rows = []
...
def _build_error_threshold_summaries(records, test_month_labels):
    ...
    for metric_name, rule_delta_key, ml_delta_key in metric_configs:
        ...
        all_nmls = sorted(set(rule_errors_by_nmls) | set(ml_errors_by_nmls), key=lambda x: str(x))
        if not all_nmls:
